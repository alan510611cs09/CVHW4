...
if __name__ == '__main__':
    ...
    # 1. Load the model architecture from the checkpoint
    print(f"Loading model from: {ckpt_path}")
    checkpoint = torch.load(ckpt_path, map_location=device)
    
    # 2. Initialize the model shell
    net = PromptIRModel().to(device)
    
    # 3. Check for EMA weights and load them, otherwise fall back to standard weights
    if 'ema_state_dict' in checkpoint and checkpoint['ema_state_dict'] is not None:
        print("--- SUCCESS: Loading EMA weights ---")
